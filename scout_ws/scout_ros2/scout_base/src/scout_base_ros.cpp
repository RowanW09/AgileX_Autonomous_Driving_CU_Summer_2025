...
ScoutBaseRos::ScoutBaseRos(std::string node_name)
  ...
  this->declare_parameter("publish_tf", rclcpp::ParameterValue(true)); // custom (ROWAN)
...
void ScoutBaseRos::LoadParameters() {
  ...
  this->get_parameter_or<bool>("publish_tf", publish_tf_, true); // custom (ROWAN)
...
void ScoutBaseRos::Run() {
  ...
  if (!is_omni_) {
    ...
    messenger->SetPublishTf(publish_tf_);  // custom (ROWAN)
