...
public:
  ...
  {
    // --- parameters ---
    this->declare_parameter("z_min", 0.05); //0.05
    this->declare_parameter("z_max", 0.4); // 0.4
    this->declare_parameter("voxel_size", 0.05); // 0.05
              ...
              const Cloud2::ConstSharedPtr in_livox)
{
  try {
    ...
    // 1) TF lookup
    auto tf_rgbd = tf_buffer_.lookupTransform(
        "base_link", in_rgbd->header.frame_id, tf2::TimePointZero);
    auto tf_livox = tf_buffer_.lookupTransform(
        "base_link", in_livox->header.frame_id, tf2::TimePointZero);

    // 2) build separate Eigen matrices
    Eigen::Matrix4f mat_rgbd =
      tf2::transformToEigen(tf_rgbd).matrix().cast<float>();
    Eigen::Matrix4f mat_livox =
      tf2::transformToEigen(tf_livox).matrix().cast<float>();

    // 3) ROS→PCL
    pcl::fromROSMsg(*in_rgbd,  *rgbd_cloud_);
    pcl::fromROSMsg(*in_livox, *livox_cloud_);

    // 4) apply each transform to its own cloud
    pcl::transformPointCloud(*rgbd_cloud_,  *rgbd_cloud_,  mat_rgbd);
    pcl::transformPointCloud(*livox_cloud_, *livox_cloud_, mat_livox);
